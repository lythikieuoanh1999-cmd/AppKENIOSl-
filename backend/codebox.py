...
import sqlite3, logging, asyncio, subprocess, tempfile, sys
...
class CodeRunIn(BaseModel):
    """Chạy code Python trực tiếp trên server (sandbox)."""
    code: str
...
def payment_create(b: PaymentIn, user=Depends(get_user)) -> dict[str, Any]:
    ...
    return {
        ...
        "credits": pkg["credits"],
        "label": pkg["label"],
        "message": "Thanh toán qua chuyển khoản ngân hàng hoặc tích hợp cổng thanh toán.",
        "bank_info": {
            "bank": "Vietcombank",
            "account": os.getenv("BANK_ACCOUNT", "1234567890"),
            "name": os.getenv("BANK_NAME", "KENIOS"),
            "content": f"KENIOS {ref}",
        },
    }
...
async def on_error(request: Request, exc: Exception):
    ...
    return JSONResponse(status_code=500,
                        content={"detail": "Lỗi server nội bộ. Vui lòng thử lại sau."})
